...
class TestDependencyIntegration(unittest.TestCase):
    ...
    def test_pynput_specific_version(self):
        ...
        # pynput uses metadata for version management
        try:
            from importlib.metadata import version

            pynput_version = version('pynput')
            self.assertIsNotNone(pynput_version)
            self.assertGreaterEqual(
                tuple(map(int, pynput_version.split('.')[:2])), (1, 7)
            )
        except ImportError:
            # Python < 3.8 fallback
            import pkg_resources

            pynput_version = pkg_resources.get_distribution('pynput').version
            self.assertIsNotNone(pynput_version)
            self.assertGreaterEqual(
                tuple(map(int, pynput_version.split('.')[:2])), (1, 7)
            )
