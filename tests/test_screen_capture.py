...
class TestScreenCaptureManager(unittest.TestCase):
    ...
    def test_capture_screen_with_fallback(self, mock_grab, mock_log_error):
        ...
        with patch.object(self.capture_manager, '_gdi_capture_with_watermark') as mock_gdi:
            ...
            mock_log_error.assert_called_with("Err-CAP", "Native screen capture failed, using GDI fallback")
    ...
    def test_capture_screen_complete_failure(self, mock_grab, mock_log_error):
        ...
        with patch.object(self.capture_manager, '_gdi_capture_with_watermark', return_value=None):
            result = self.capture_manager.capture_screen()
