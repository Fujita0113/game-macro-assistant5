...
def test_macro_data_structures():
    ...
    screen_condition = ScreenCondition(
        image_data=image_data, region=(10, 20, 30, 40), threshold=0.8, timeout=5.0
    )
...
def test_image_editor_classes():
    ...
    try:
        from src.ui import image_editor

        # Access the module to avoid unused import warning
        _ = image_editor.ImageEditor
...
def test_region_selection_logic():
    ...
    scale_factor = min(
        display_size[0] / original_size[0], display_size[1] / original_size[1]
    )
