...
class DragDropCanvas(tk.Canvas):
    ...
    def _get_block_text(self, operation: OperationBlock) -> str:
        ...
        if operation.operation_type == OperationType.MOUSE_CLICK:
            ...
            if operation.keyboard_op:
                modifiers = (
                    '+'.join(operation.keyboard_op.modifiers) + '+'
                    if operation.keyboard_op.modifiers
                    else ''
                )
    ...
    def _update_scroll_region(self):
        ...
        if self.blocks:
            height = self.margin * 2 + len(self.blocks) * (
    ...
    def _update_drop_indicator(self, y_pos):
        ...
        target_index = max(
            ...
            min(
                len(self.blocks),
                int(
                    (y_pos - self.margin + self.block_spacing // 2)
                    // (self.block_height + self.block_spacing)
                ),
            ),
        )
        ...
        indicator_y = (
            self.margin + target_index * (self.block_height + self.block_spacing) - 5
        )
        ...
        if self.drag_data['item']:
            ...
            target_index = max(
                ...
                min(
                    len(self.blocks),
                    int(
                        (event.y - self.margin + self.block_spacing // 2)
                        // (self.block_height + self.block_spacing)
                    ),
                ),
            )
...
class VisualEditor:
    ...
    def _setup_ui(self):
        ...
        title_label = ttk.Label(
            main_frame, text='マクロ操作ブロック', font=('Arial', 12, 'bold')
        )
        ...
        self.status_label = ttk.Label(
            toolbar_frame, text='操作をドラッグ&ドロップで並び替えできます'
        )
    ...
    def _on_blocks_reordered(self, from_index: int, to_index: int):
        ...
        if self.macro_recording:
            ...
            self.status_label.config(
                text=f'ブロックを移動しました: {from_index + 1} → {to_index + 1}'
            )
