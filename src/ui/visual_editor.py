...
class OperationBlockWidget:
    ...
    def __init__(self, parent: tk.Widget, block: OperationBlock, 
    ...
    def _setup_content(self):
        ...
        if self.block.operation_type == OperationType.MOUSE_CLICK and self.block.mouse_op:
            self._setup_mouse_content(content_frame)
        elif self.block.operation_type == OperationType.KEY_PRESS and self.block.keyboard_op:
            self._setup_keyboard_content(content_frame)
        elif self.block.operation_type == OperationType.SCREEN_CONDITION and self.block.screen_condition:
    ...
    def _setup_keyboard_content(self, parent: tk.Widget):
        ...
        modifiers_text = " + ".join(keyboard_op.modifiers) if keyboard_op.modifiers else ""
        key_text = f"{modifiers_text} + {keyboard_op.key}" if modifiers_text else keyboard_op.key
    ...
    def _setup_screen_condition_content(self, parent: tk.Widget):
        ...
        if screen_condition.image_data:
            try:
                ...
                image_label.bind("<Double-Button-1>", lambda e: self._on_image_double_click())
        ...
        # Condition info
        region_text = "全画面" if not screen_condition.region else f"領域: {screen_condition.region}"
...
class VisualEditor:
    ...
    def _setup_scrollable_frame(self):
        ...
        scrollbar = ttk.Scrollbar(self.scroll_frame, orient=tk.VERTICAL, command=self.canvas.yview)
    ...
    def _create_block_widgets(self):
        ...
        for i, block in enumerate(self.recording.operations):
            block_widget = OperationBlockWidget(
                ...
                block,
                on_image_edit=self._on_block_image_edit
            )
    ...
    def _on_block_image_edit(self, block_widget: OperationBlockWidget):
        ...
        if (block_widget.block.operation_type != OperationType.SCREEN_CONDITION or
            not block_widget.block.screen_condition or
            not block_widget.block.screen_condition.image_data):
            return
    ...
    @staticmethod
    def open_visual_editor(recording: MacroRecording, parent: Optional[tk.Widget] = None) -> "VisualEditor":
        """
