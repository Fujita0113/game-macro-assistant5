...
class ImageEditor:
    ...
    def __init__(self, parent: tk.Widget, image_data: bytes, 
    ...
    def _on_mouse_down(self, event):
        ...
        if (0 <= canvas_x <= self.display_width and 
            0 <= canvas_y <= self.display_height):
    ...
    def _on_confirm_click(self):
        ...
        if selection is None:
            self._show_error("有効な領域を選択してください。\n（最小サイズ: 5x5ピクセル）")
            return
    ...
    @staticmethod
    def open_image_editor(parent: tk.Widget, image_data: bytes, 
                         on_confirm: Optional[Callable[[Tuple[int, int, int, int]], None]] = None) -> "ImageEditor":
        """
