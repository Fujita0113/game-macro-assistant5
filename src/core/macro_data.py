...
class OperationBlock:
    ...
    def __post_init__(self):
        ...
        elif self.operation_type in [OperationType.KEY_PRESS, OperationType.KEY_RELEASE]:
            if self.keyboard_op is None:
                raise ValueError(f"KeyboardOperation required for {self.operation_type}")
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "OperationBlock":
        ...
        mouse_op = MouseOperation.from_dict(data["mouse_op"]) if "mouse_op" in data else None
        keyboard_op = KeyboardOperation.from_dict(data["keyboard_op"]) if "keyboard_op" in data else None
        screen_condition = ScreenCondition.from_dict(data["screen_condition"]) if "screen_condition" in data else None
...
class MacroRecording:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "MacroRecording":
        ...
        operations = [OperationBlock.from_dict(op_data) for op_data in data.get("operations", [])]
...
def create_key_operation(
    ...
    keyboard_op = KeyboardOperation(
        key=key,
        action=action,
        modifiers=modifiers
    )
    
    operation_type = OperationType.KEY_PRESS if action == "press" else OperationType.KEY_RELEASE
