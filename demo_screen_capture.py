...
def test_basic_capture():
    ...
    if screenshot:
        print(
            f'✅ 成功: スクリーンショットが保存されました (サイズ: {screenshot.size})'
        )
...
def test_error_simulation():
    ...
    print(
        '📋 ログファイルを確認してErr-CAPエラーコードが記録されているか確認してください'
    )
...
def fullscreen_test():
    ...
    print('\n手順:')
    print(
        '1. このメッセージの後、フルスクリーンアプリケーション（ゲーム、動画プレーヤーなど）を起動'
    )
...
def main():
    ...
    parser.add_argument(
        '--fullscreen', action='store_true', help='フルスクリーンテストモード'
    )
    ...
    if args.manual:
        ...
        print('\n📁 生成されたファイル:')
        for filename in [
            'test_basic_capture.png',
            'test_region_capture.png',
            'test_watermark.png',
            'screenshots/',
        ]:
